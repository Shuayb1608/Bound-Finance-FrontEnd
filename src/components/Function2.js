...
export async function stakeTokens(amount) {
    ...
    let amountheldbyaccount = await sai.methods.balanceOf(account).call()
    if(amount > amountheldbyaccount) {
        Swal.close();
        Swal.fire({
            title: 'warning',
...
export async function Bust(amount) {
    ...
    try {
       ...
       const tokenContract = await getTokenContract3();
       const amounttowei = etherToWei(amount);
       await tokenContract.methods.approve(TAP_ADDRESS, amounttowei).send({from: account});
        await tap.methods.bust(amounttowei).send({from: account});

        Swal.fire({
            icon: 'success',
            title:  'You have pruchased some SKR with BCK',
            text: 'The liquidation process is complete.',
        });
        ...
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Something went wrong with Liquidations!'
        });
}
}
